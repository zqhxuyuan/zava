...
import com.zqh.java.kvstore.KVStoreFactory;
import com.zqh.java.kvstore.Options;
import IntHolder;
import BplusTree.TreeEntry;
import BplusTreeFile;
...
###### Test-1 (BenchMarkDiskStore): Registry { count=1e6, datalen=256bytes } BlockStore { blockSize=512 (2reg/block), fileSize=250MB } StreamStore { outBufferSize=0x10000, align=true, fileSize=256MB }
...
###### Test-2 (BenchMarkMemoryStructures): Registry { count=2e6, datalen=256bytes } BplusTreeMemory { key=Integer, b-order=511 } IntHashMap { initialSize=(2e6 * 2) }
