 ...
 */
public class UnsafeTableValueHistory extends TableValueHistory {
    private final LinkedList<TableValue> entries = 
	new LinkedList<>();

    public UnsafeTableValueHistory() {
	super();
    }

    public UnsafeTableValueHistory(TableValueHistory oldHistory,
	...
	return TableBucketCollapser.collapseHistory(iterator());
    }
	...
	if(oldHistory != null) {
	    ...
	    return new StreamIterator.ReverseIterator<TableValue>(entries.listIterator(entries.size()), 
								 entries.size());
	}
    }
